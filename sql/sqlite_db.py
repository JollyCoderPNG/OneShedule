...
async def create_profile(user_id):
    user = cur.execute(
        "SELECT 1 FROM profile WHERE user_id == ?",
        (user_id,)).fetchone()
    if not user:
        cur.execute(
...
async def update_frm(user_id, frm):
    cur.execute(
        "UPDATE profile SET frm = ? WHERE user_id == ?",
        (frm, user_id))
    db.commit()

async def update_grup(user_id, grup):
    cur.execute(
        "UPDATE profile SET grup = ? WHERE user_id == ?",
        (grup, user_id))
    db.commit()

async def update_half(user_id, half):
    cur.execute(
        "UPDATE profile SET half = ? WHERE user_id == ?",
        (half, user_id))
    db.commit()

async def update_ring(user_id, ring):
    cur.execute(
        "UPDATE profile SET ring = ? WHERE user_id == ?",
        (ring, user_id))
    db.commit()
